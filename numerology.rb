puts "What's your birthday? (please use MMDDYYYY format!)"
...
def birthpath_number (birthdate)
    ...
    number=number.to_s
    if number.length > 1
        number1 = number[0].to_i+number[1].to_i
        else number1 = number.to_i
    end
    number1=number1.to_s
    if number1.length > 1
        number2 = number1[0].to_i+number1[1].to_i
        else number2 = number1.to_i
    end
    number2
end

number = birthpath_number(birthdate)
...
def horoscope (number)
    case number
        ...
        when 9
            ...
            message = "no comprehendo"
    end
    message = message
end

message = horoscope(number)
